    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Some(node_match) =
                pass_named.then(|| self.matcher.match_node(node)).flatten()
            {
...
impl<'tree, L: LanguageExt> Level<'tree, L> {
    pub fn new(node: &Node<'tree, StrDoc<L>>) -> Self {
        ...
        Self { deque, cursor, root: node.root }
    }
}
