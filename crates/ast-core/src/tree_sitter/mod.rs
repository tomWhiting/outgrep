...
impl<L: LanguageExt> StrDoc<L> {
    pub fn try_new(src: &str, lang: L) -> Result<Self, String> {
        ...
        let tree = parse_lang(|p| p.parse(src.as_bytes(), None), ts_lang)
...
impl<'r> SgNode<'r> for Node<'r> {
    ...
    fn children(&self) -> impl ExactSizeIterator<Item = Self> {
        ...
        NodeWalker { cursor, count: self.child_count() }
    }
...
pub fn perform_edit<S: ContentExt>(
    tree: &mut Tree,
    input: &mut S,
    edit: &Edit<S>,
) -> InputEdit {
...
impl<L: LanguageExt> Root<StrDoc<L>> {
    ...
    pub fn get_injections<F: Fn(&str) -> Option<L>>(
        &self,
        get_lang: F,
    ) -> Vec<Self> {
        let root = self.root();
        ...
        let roots = range
            ...
            .filter_map(|(lang, ranges)| {
                ...
                Some(Self {
                    doc: StrDoc { src: self.doc.src.clone(), lang, tree },
                })
            })
            .collect();
        roots
...
mod test {
    ...
    fn test_edit() -> Result<(), TSParseError> {
        ...
        let tree2 =
            parse_lang(|p| p.parse(&src, Some(&tree)), Tsx.get_ts_language())?;
        assert_eq!(
