...
pub(super) fn match_node_impl<'tree, D: Doc>(
    ...
    match &goal {
        ...
        P::Terminal { text, kind_id, is_named } => match strictness
            .match_terminal(*is_named, text, *kind_id, candidate)
        {
        ...
        P::Internal { kind_id, children, .. } => {
            let kind_matched = strictness.should_skip_kind()
                || kind_utils::are_kinds_matching(
                    *kind_id,
                    candidate.kind_id(),
                );
            if !kind_matched {
            ...
            match match_nodes_impl_recursive(
                children,
                cand_children,
                agg,
                strictness,
...
fn match_nodes_impl_recursive<'tree, D: Doc>(
    ...
    loop {
        ...
        if goal_children.peek().is_none() {
            // all goal found
            let has_trailing =
                cand_children.all(|n| strictness.should_skip_trailing(&n));
...
mod test {
    ...
    fn match_tree(
        ...
        let kind = KindMatcher::from_id(
            kind.into_iter().next().expect("should have kind") as u16,
        );
    ...
    fn test_signature_match() {
        matched("import $A from 'lib'", "import A from \"lib\"", M::Signature);
