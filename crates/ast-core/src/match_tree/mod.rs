...
pub fn match_end_non_recursive(
    goal: &Pattern,
...
pub fn does_node_match_exactly<D: Doc>(
    goal: &Node<D>,
    candidate: &Node<D>,
) -> bool {
...
mod test {
    ...
    fn test_nested_match() {
        ...
        test_match(
            "const a = 123",
            "class A { constructor() {const a= 123;}}",
        );
        test_match(
            "const a = 123",
    ...
    fn find_end_recursive(
        goal: &Pattern,
        node: Node<StrDoc<Tsx>>,
    ) -> Option<usize> {
        match_end_non_recursive(goal, node.clone()).or_else(|| {
            node.children().find_map(|sub| find_end_recursive(goal, sub))
        })
    }
