...
impl<P: Matcher> Matcher for All<P> {
    fn match_node_with_env<'tree, D: Doc>(
        ...
        let mut new_env = Cow::Borrowed(env.as_ref());
        let all_satisfied = self.patterns.iter().all(|p| {
            p.match_node_with_env(node.clone(), &mut new_env).is_some()
        });
        if all_satisfied {
            *env = Cow::Owned(new_env.into_owned());
            Some(node)
    ...
    P2: Matcher,
{
    fn match_node_with_env<'tree, D: Doc>(
        ...
        let mut new_env = Cow::Borrowed(env.as_ref());
        if let Some(ret) =
            self.pattern1.match_node_with_env(node.clone(), &mut new_env)
        {
            *env = Cow::Owned(new_env.into_owned());
            Some(ret)
        } else {
    ...
    P: Matcher,
{
    fn match_node_with_env<'tree, D: Doc>(
        ...
        self.not.match_node_with_env(node.clone(), env).xor(Some(node))
    }
}
...
impl<M: Matcher> Op<M> {
    pub fn every(pattern: M) -> Op<And<M, MatchAll>> {
        Op { inner: And { pattern1: pattern, pattern2: MatchAll } }
    }
    pub fn either(pattern: M) -> Op<Or<M, MatchNone>> {
        Op { inner: Or { pattern1: pattern, pattern2: MatchNone } }
    }
...
impl<M: Matcher, N: Matcher> Op<And<M, N>> {
    pub fn and<O: Matcher>(self, other: O) -> Op<NestedAnd<M, N, O>> {
        Op { inner: And { pattern1: self.inner, pattern2: other } }
    }
}
...
impl<M: Matcher, N: Matcher> Op<Or<M, N>> {
    pub fn or<O: Matcher>(self, other: O) -> Op<NestedOr<M, N, O>> {
        Op { inner: Or { pattern1: self.inner, pattern2: other } }
    }
}
...
mod test {
    ...
    fn find_all(matcher: impl Matcher, code: &str) -> Vec<String> {
        let node = Root::str(code, Tsx);
        node.root().find_all(matcher).map(|n| n.text().to_string()).collect()
    }

    #[test]
    fn test_or() {
        let matcher = Or { pattern1: "let a = 1", pattern2: "const b = 2" };
    ...
    fn test_multiple_match() {
        let sequential =
            find_all("$A + b", "let f = () => a + b; let ff = () => c + b");
    ...
    */
    ...
    fn test_and_kinds() {
        ...
        let matcher =
            Op::every("let a = $_".t()).and(Op::not("let a = 123".t()));
        assert_eq!(matcher.potential_kinds().map(|v| v.len()), Some(1));
        let matcher =
            Op::every(Op::not("let a = $_".t())).and("let a = 123".t());
        assert_eq!(matcher.potential_kinds().map(|v| v.len()), Some(1));
        ...
        // two None kinds
        let matcher = Op::every(Op::not("let a = $_".t()))
            .and(Op::not("let a = 123".t()));
    ...
    fn test_or_kinds() {
        // union None kinds
        let matcher =
            Op::either("let a = $_".t()).or(Op::not("let a = 123".t()));
        assert_eq!(matcher.potential_kinds(), None);
        let matcher =
            Op::either(Op::not("let a = $_".t())).or("let a = 123".t());
        assert_eq!(matcher.potential_kinds(), None);
        // union Same kinds
        ...
        // two None kinds
        let matcher = Op::either(Op::not("let a = $_".t()))
            .or(Op::not("let a = 123".t()));
        assert_eq!(matcher.potential_kinds(), None);
    }
    ...
    fn test_or_revert_env() {
        let matcher =
            Op::either(Op::every("foo($A)".t()).and("impossible".t()))
                .or("foo($B)".t());
