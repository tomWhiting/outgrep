use super::indent::{
    extract_with_deindent, get_indent_at_offset, indent_lines,
    DeindentedExtract,
};
...
fn create_template(
    ...
    while let Some(i) = tmpl[len + offset..].find(mv_char) {
        if let Some((meta_var, skipped)) = split_first_meta_var(
            &tmpl[len + offset + i..],
            mv_char,
            transforms,
        ) {
            ...
            let indent = get_indent_at_offset::<String>(
...
if (true) {
  ...
  c(
    ...
    fn test_str_replace(
        ...
        let roots: Vec<_> =
            vars.iter().map(|(v, p)| (v, Tsx.ast_grep(p))).collect();
    ...
    fn test_replace_rewriter() {
        let tf = TemplateFix::with_transform(
            "if (a)\n  $A",
            &Tsx,
            &["A".to_string()],
        );
