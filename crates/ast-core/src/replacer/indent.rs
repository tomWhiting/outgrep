...
pub fn indent_lines<C: Content>(
    ...
    match original_indent.cmp(&indent) {
        ...
        Ordering::Greater => {
            Cow::Owned(remove_indent::<C>(original_indent - indent, lines))
        }
...
fn remove_indent<C: Content>(
    indent: usize,
    src: &[C::Underlying],
) -> Vec<C::Underlying> {
    let indentation: Vec<_> =
        std::iter::repeat(get_space::<C>()).take(indent).collect();
    let new_line = get_new_line::<C>();
    let lines: Vec<_> = src
        .split(|b| *b == new_line)
...
mod test {
    ...
    fn test_deindent(source: &str, expected: &str, offset: usize) {
        ...
        let start =
            source[offset..].chars().take_while(|n| n.is_whitespace()).count()
                + offset;
        let trailing_white =
            source.chars().rev().take_while(|n| n.is_whitespace()).count();
...
";
    ...
    fn test_replace_with_indent(
        ...
        let replace_lines =
            DeindentedExtract::MultiLine(inserted.as_bytes(), 0);
        let indent =
            get_indent_at_offset::<String>(&target.as_bytes()[..start]);
