...
impl Position {
    pub fn new(line: usize, byte_column: usize, byte_offset: usize) -> Self {
        Self { line, byte_column, byte_offset }
    }
...
impl<D: Doc> Root<D> {
    ...
    pub fn root(&self) -> Node<D> {
        Node { inner: self.doc.root_node(), root: self }
    }
...
impl<'r, D: Doc> Node<'r, D> {
    ...
    pub fn parent(&self) -> Option<Self> {
        ...
        Some(Node { inner, root: self.root })
    }

    pub fn children(&self) -> impl ExactSizeIterator<Item = Node<'r, D>> + '_ {
        self.inner.children().map(|inner| Node { inner, root: self.root })
    }
    ...
    pub fn field_children(
        &self,
        name: &str,
    ) -> impl Iterator<Item = Node<'r, D>> + '_ {
        let field_id = self.lang().field_to_id(name);
        self.inner
            .field_children(field_id)
            .map(|inner| Node { inner, root: self.root })
    }
    ...
    pub fn ancestors(&self) -> impl Iterator<Item = Node<'r, D>> + '_ {
        ...
        self.inner.ancestors(root).map(|inner| Node { inner, root: self.root })
    }
    #[must_use]
    ...
    pub fn next_all(&self) -> impl Iterator<Item = Node<'r, D>> + '_ {
        self.inner.next_all().map(|inner| Node { inner, root: self.root })
    }
    ...
    pub fn prev_all(&self) -> impl Iterator<Item = Node<'r, D>> + '_ {
        self.inner.prev_all().map(|inner| Node { inner, root: self.root })
    }

    pub fn dfs<'s>(&'s self) -> impl Iterator<Item = Node<'r, D>> + 's {
        self.inner.dfs().map(|inner| Node { inner, root: self.root })
    }
...
mod test {
    ...
    fn test_children() {
        ...
        let texts: Vec<_> =
            children[0].children().map(|c| c.text().to_string()).collect();
