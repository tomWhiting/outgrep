...
impl<'t, D: Doc> MetaVarEnv<'t, D> {
    ...
    pub fn add_label(&mut self, label: &str, node: Node<'t, D>) {
        self.multi_matched.entry(label.into()).or_default().push(node);
    }
    ...
    pub fn get_matched_variables(
        &self,
    ) -> impl Iterator<Item = MetaVariable> + use<'_, 't, D> {
        let single = self
            .single_matched
            .keys()
            .cloned()
        ...
        let transformed = self
            ...
            .map(|n| MetaVariable::Capture(n, false));
        let multi =
            self.multi_matched.keys().cloned().map(MetaVariable::MultiCapture);
    ...
    pub fn insert_transformation(
        ...
        let node = match var {
            ...
            MetaVariable::MultiCapture(vs) => {
                self.multi_matched.get(vs).and_then(|vs| vs.first())
            }
            _ => None,
        };
        let deindented = if let Some(v) = node {
            formatted_slice(&slice, v.get_doc().get_source(), v.range().start)
                .to_vec()
        } else {
            slice
        };
    ...
    match var {
        MetaVariable::Capture(n, _) => {
            if let Some(node) = env.get_match(n) {
                let bytes =
                    node.get_doc().get_source().get_range(node.range());
                Some(bytes)
...
mod test {
    ...
    fn test_multi_var_match() {
        let grep = Tsx
            .ast_grep("if (true) { a += 1; b += 1 } else { a += 1; b += 1 }");
        let node = grep.root();
    ...
    fn test_multi_var_match_with_trailing() {
        let grep =
            Tsx.ast_grep("if (true) { a += 1; } else { a += 1; b += 1 }");
        let node = grep.root();
        let not_found = node.find("if (true) { $$$A } else { $$$A }");
        assert!(not_found.is_none());
        let grep =
            Tsx.ast_grep("if (true) { a += 1; b += 1; } else { a += 1 }");
        let node = grep.root();
        let not_found = node.find("if (true) { $$$A } else { $$$A }");
        assert!(not_found.is_none());
    }
