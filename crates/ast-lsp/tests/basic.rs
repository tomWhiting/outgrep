use outgrep_ast_config::{
    from_yaml_string, GlobalRules, RuleCollection, RuleConfig,
};
...
",
    ...
    let rc: RuleCollection<SupportLang> =
        RuleCollection::try_new(vec![config]).unwrap();
    let rc_result: std::result::Result<_, String> = Ok(rc);
    let (service, socket) =
        LspService::build(|client| Backend::new(client, base, rc_result))
            .finish();
...
pub async fn request_code_action_to_lsp(
    ...
    let mut buf = vec![0; 1024];
    req_client.write_all(req(code_action_request).as_bytes()).await.unwrap();
    let _ = resp_client.read(&mut buf).await.unwrap();
...
fn test_code_action() {
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        let buf =
            request_code_action_to_lsp(&mut req_client, &mut resp_client)
                .await;
...
fn test_execute_apply_all_fixes() {
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        let buf =
            request_execute_command_to_lsp(&mut req_client, &mut resp_client)
                .await;
