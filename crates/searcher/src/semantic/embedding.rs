...
use ort::{Session, SessionBuilder, Environment, Value, GraphOptimizationLevel};
...
impl OnnxEmbedder {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if !model_path.exists() || !tokenizer_path.exists() {
            return Err("Model files not found. Run from project root directory.".into());
        }
        
        // Create environment
        let environment = Arc::new(Environment::builder()
            .with_name("outgrep-semantic")
            .build()?);
    ...
    pub fn embed(&self, text: &str) -> Result<Embedding, Box<dyn std::error::Error>> {
        ...
        let token_type_ids_2d: Array2<i64> = Array2::zeros((1, input_ids.len()));
        ...
        if (norm_check - 1.0).abs() > 0.001 {
            eprintln!("WARNING: Embedding not properly normalized! Norm: {:.6}", norm_check);
        }
        
        // Ensure we always return exactly 384 dimensions
        if pooled.len() != 384 {
            eprintln!("WARNING: ONNX model produced {} dimensions, expected 384", pooled.len());
...
pub fn cosine_similarity(a: &Embedding, b: &Embedding) -> f32 {
    ...
    let dot_product: f32 = a.vector.iter()
        .zip(b.vector.iter())
        .map(|(x, y)| x * y)
        .sum();
